    ...
    render() {
        ...
        return (
            ...
            <div className="datatable-footer"
                 style={{opacity: this.props.counts != 0 ? '1' : '0', borderTop: '0 red solid', marginTop: "-76px"}}>
                <div className="dataTables_info" id="DataTables_Table_2_info" role="status" aria-live="polite"
                     style={{display: this.props.sortType && this.props.sortType.length > 0 ? "inline-block" : "none"}
                     }>
                    <span
                        style={{fontWeight: "bold", fontSize: "12px", float: "left"}}>{Current_Lang.others.sort}</span>
                    <select id={this.sortTypeID}
                            style={{marginTop: '-8px', marginLeft: "10px", width: "auto", float: "left"}}
                            className="form-control input-xs"
                            onChange={this._changeSortType ? this._changeSortType.bind(this) : ""}>
                        {sortType}
                    </select>
                    <select id={this.sortLogicID}
                            style={{marginTop: '-8px', marginLeft: "10px", width: "auto", float: "left"}}
                            className="form-control input-xs"
                            onChange={this._changeSortType ? this._changeSortType.bind(this) : ""}>
                        <option value={"desc"}>{Current_Lang.others.desc}</option>
                        <option value={"asc"}>{Current_Lang.others.asc}</option>
                    </select>
                </div>
                <div style={{display: this.props.operation == false ? "none" : "inline-block", margin: "0"}}
                ...
                <div className="dataTables_info" id="DataTables_Table_2_info" role="status" aria-live="polite"
                    ...
                    <select id={this.props.perPageID ? this.props.perPageID : "per_page"}
                            style={{width: '70px', marginTop: '-8px'}} className="form-control input-xs pageSelect"
